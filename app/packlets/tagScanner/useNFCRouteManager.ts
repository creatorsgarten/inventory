...
const pathRegex = /^\/tags\/([A-Z]{2}\d{5})$/
...
export const useNFCRouteManager = () => {
  ...
  useEffect(() => {
    ...
    if (nfcMonitor === null) {
      // reset atom if location is not a matched pattern
      if (location.pathname.match(pathRegex) === null) {
      ...
      // do a redirect if the path matches patterns
        ...
        searchParams.append(nfcMonitor!.searchKey, location.pathname.match(pathRegex)![1])
